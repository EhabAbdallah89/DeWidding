export function normalizeText(value){ return value.trim() }
...
export function formatStatusLabel(status){ if(status==='approved') return 'موافق عليه'; if(status==='pending') return 'بانتظار الموافقة'; if(status==='rejected') return 'مرفوض'; return status }
