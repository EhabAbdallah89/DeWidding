...
import { formatDisplayDate, formatStatusLabel, isPastEvent } from '../utils/eventUtils'

function EventTable({ events, currentUser, onEditEvent, onDeleteEvent, onApproveEvent, onRejectEvent, onToggleMyEvent, myEventIds = [], showCreatedBy = false, createdByNameMap = {} }) {
  if (events.length === 0) {
      ...
      <table>
        ...
        <tbody>
          {events.map((event) => {
            ...
            const isSaved = myEventIds.includes(event.id)
            return (
              <tr key={event.id} className={eventIsPast ? 'row-past' : 'row-upcoming'}>
                ...
                {showCreatedBy && <td>{createdByNameMap[event.createdByUserId] || event.createdByUserId || '-'}</td>}
                <td>
                  {currentUser ? (
                    <button className={isSaved ? 'ghost-btn' : 'primary-btn'} onClick={() => onToggleMyEvent?.(event.id)}>
                      {isSaved ? 'إزالة' : 'أضف إلى أعراسي'}
                    </button>
                  ) : '—'}
                </td>
